...
errors=(); warnings=(); checks_json="{}"
add_error(){ errors+=("$*"); }
add_warning(){ warnings+=("$*"); }
check_cmd(){ command -v "$1" >/dev/null 2>&1 || add_error "缺少命令：$1"; }
[[ -w "$PROJECT_DIR" ]] || add_error "项目目录不可写：$PROJECT_DIR"
if [[ -r /etc/os-release ]]; then . /etc/os-release; [[ "${ID,,}" =~ (centos|rhel|rocky|almalinux) ]] || add_error "暂定仅支持 CentOS/RHEL 系系统，当前：${PRETTY_NAME:-unknown}"; else add_error "无法读取 /etc/os-release"; fi
check_cmd curl; check_cmd git; check_cmd docker
...
if [[ -z "${PROJECT_BOOTSTRAP_TOKEN:-}" ]]; then add_error "PROJECT_BOOTSTRAP_TOKEN 未配置"; fi
