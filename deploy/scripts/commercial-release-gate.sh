...
run_step "CI/CD 工作区权限位自愈契约检查（Docker 工具容器）" cp_python_tool deploy/scripts/check-deploy-worktree-contract.py
...
run_step "前端 package-lock 完整性检查（Docker 工具容器）" cp_python_tool deploy/scripts/check-frontend-lockfile-integrity.py
