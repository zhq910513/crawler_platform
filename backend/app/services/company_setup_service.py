...
class CompanySetupService:
    ...
    def _counts(self, company_id: int) -> dict[str, Any]:
        ...
        task_definitions = self.db.scalar(select(func.count()).select_from(CrawlerProjectTaskDefinition).where(CrawlerProjectTaskDefinition.company_id == company_id, CrawlerProjectTaskDefinition.definition_status == "AVAILABLE")) or 0
