...
class CrawlerImageVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "image_digest", name="uk_project_image_digest"),
        Index("idx_image_project_built", "project_id", "built_at"),
    )
...
class CrawlerReleaseChannel(Base, TimestampMixin):
    ...
    image_version_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("crawler_image_version.image_version_id"), nullable=False)
...
class CrawlerTask(Base, TimestampMixin):
    ...
    executor_type: Mapped[str] = mapped_column(String(30), default="PYTHON_METHOD", nullable=False)
...
class CrawlerTaskRuntime(Base, TimestampMixin):
    ...
    image_policy: Mapped[str] = mapped_column(String(30), default="PINNED", nullable=False)
...
class CrawlerTaskRun(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("idx_run_task_created", "task_id", "created_at"),
