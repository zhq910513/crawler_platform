...
def test_build_center_logs_docker_context_pull_and_error_details() -> None:
    ...
    assert 'DOCKER_PULL' in service
    assert 'def _prepare_base_images' in service
    assert 'def _docker_error_detail' in service
    assert 'Docker Engine API 构建失败' in service and 'baseImages' in service
    assert 'crawler_project_docker_pull_attempts' in config
    assert 'crawler_project_docker_pull_retry_seconds' in config
