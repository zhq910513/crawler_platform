...
def test_cicd_one_click_guide_and_public_templates() -> None:
    ...
    company = client.post('/api/v1/companies', headers=headers, json={'companyCode': 'hc_cicd_1032', 'companyName': 'H公司CI1032'}).json()['data']
    ...
    assert 'CRAWLER_CONTROL_BASE_URL: "https://control.example.com"' in guide['workflowContent']
...
def test_spider_release_register_helper_builds_payload_from_crawler_project_json(tmp_path, monkeypatch) -> None:
    ...
    spec = importlib.util.spec_from_file_location('spider_release_register_1032', helper_path)
...
def test_discovered_project_can_resolve_company_by_company_code() -> None:
    ...
    client = TestClient(app)
    _, headers = login(client)
    company_a = client.post('/api/v1/companies', headers=headers, json={'companyCode': 'company_a_1032', 'companyName': '公司A1032'}).json()['data']
    company_b = client.post('/api/v1/companies', headers=headers, json={'companyCode': 'company_b_1032', 'companyName': '公司B1032'}).json()['data']
