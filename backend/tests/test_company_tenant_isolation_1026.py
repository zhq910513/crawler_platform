...
def test_normal_user_is_company_scoped_for_settings_and_agent_join() -> None:
    ...
    join = client.post('/api/v1/servers/agent-join-tokens', headers=user_headers, json={'companyId': company_a['companyId'], 'serverCode': 'tenant26a-node', 'serverName': '租户A节点', 'agentCode': 'tenant26a-agent', 'maxContainerSlots': 2, 'platformUrl': 'http://10.1.0.13:8080', 'installTarget': 'REMOTE'})
    assert join.status_code == 200
    assert join.json()['data']['companyId'] == company_a['companyId']
    cross_join = client.post('/api/v1/servers/agent-join-tokens', headers=user_headers, json={'companyId': company_b['companyId'], 'serverCode': 'tenant26b-node', 'serverName': '租户B节点', 'agentCode': 'tenant26b-agent', 'maxContainerSlots': 2, 'platformUrl': 'http://10.1.0.13:8080', 'installTarget': 'REMOTE'})
