# Agent 接入、项目部署与离线兜底说明（1.0.17）

## 角色边界

- 平台超级管理员：维护公司、项目、Release、CI/CD、密钥与全局发布策略。
- 公司运维：通过平台生成的命令安装 Agent，查看服务器状态，部署项目 Release 到本公司服务器，调整部分调度。
...
2. 选择公司，填写服务器编码、服务器名称、Agent 编码、最大并发、工作目录、标签与能力。
3. 平台生成一次性安装命令。
4. 公司运维复制命令到目标服务器执行。
...
Agent 默认只主动连接平台和镜像仓库，不要求服务器开放公网入站端口。

## 已有 Agent 部署第二个项目

已有 Agent 的服务器不需要重新安装 Agent，也不需要拉项目源码。流程为：
