...
const buildBlockerData = computed<Record<string, unknown>>(() => {
  const blocker = publishBlockers.value.find((item) => String(item.step || item.key || '') === 'build')
  const data = (blocker?.data || {}) as Record<string, unknown>
  return data
})
...
.external-release-card { margin: 12px 0; padding: 12px; border: 1px solid #fed7aa; background: #fff7ed; border-radius: 12px; }
...
.guide-row span { color: #64748b; }
.compact-command { margin-top: 8px; }
