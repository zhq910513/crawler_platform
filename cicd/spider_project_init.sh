...
release_channel="${CRAWLER_RELEASE_CHANNEL:-${RELEASE_CHANNEL:-stable}}"
...
case "$provider" in
  github|gha)
    ...
    if [ -f "$target" ] && [ "$force" != "1" ]; then echo "$target 已存在。如需覆盖：CRAWLER_INIT_FORCE=1" >&2; exit 3; fi
...
echo "下一步：git add . && git commit -m '接入 crawler 自动构建发布' && git push"
